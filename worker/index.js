...
// Calls the Anthropic /v1/messages endpoint with automatic retry on transient
// failures (502, 503, 504, 529 — gateway/overload errors that often resolve
// within seconds). Uses exponential backoff: wait 1s, then 2s, then 4s between
// retries. Returns the final response (success or last failure) plus metadata
// about whether retries happened.
//
// Does NOT retry on:
//   - 200/2xx (success)
//   - 400 (bad request — won't fix itself)
//   - 401/403 (auth — won't fix itself)
//   - 413 (payload too large — won't fix itself)
//   - 429 (rate limit — we want the user to back off, not retry)
//
// Retries on:
//   - 502 Bad Gateway (most common Brain failure cause)
//   - 503 Service Unavailable
//   - 504 Gateway Timeout
//   - 529 Overloaded (Anthropic-specific transient overload)
//   - Network errors (fetch throws — usually means transient connection issue)
//
// Returns: { response, retriesAttempted, finalStatus }
async function callAnthropicWithRetry(env, requestBody, label = 'unknown') {
  const RETRY_DELAYS_MS = [1000, 2000, 4000]; // 3 retries total, exponential backoff
  ...
  for (let attempt = 0; attempt < totalAttempts; attempt++) {
    ...
    if (attempt > 0) {
      ...
      console.log(`[${label}] Retry ${attempt}/${RETRY_DELAYS_MS.length} after ${delay}ms (last status: ${lastResponse?.status || 'network error'})`);
    ...
    try {
      ...
      if (response.ok) {
        ...
        return { response, retriesAttempted, finalStatus: response.status };
      }

      // Non-retryable error — bail out immediately, don't waste retries
      if (!RETRYABLE_STATUSES.has(response.status)) {
        return { response, retriesAttempted, finalStatus: response.status };
      }

      // Retryable error — log and continue loop to next attempt
      // (response body will be consumed by next attempt; we don't read it here
      // because cloning the response in workers is awkward. The final attempt's
      // response is returned for the caller to read its error body.)
      console.warn(`[${label}] Attempt ${attempt + 1}/${totalAttempts} got status ${response.status}, will retry if attempts remain`);

    } catch (e) {
      // Network error (fetch threw) — retry
      lastError = e;
      console.warn(`[${label}] Attempt ${attempt + 1}/${totalAttempts} failed with network error: ${e.message}`);
    }
  }

  // All retries exhausted. Return whatever the last result was.
  // If we had a response (even an error one), return it so the caller can read the error body.
  // If we had a network error, throw it so the caller's try/catch handles it.
  if (lastResponse) {
    console.error(`[${label}] All ${totalAttempts} attempts exhausted, last status: ${lastResponse.status}`);
    return { response: lastResponse, retriesAttempted, finalStatus: lastResponse.status };
  }
  console.error(`[${label}] All ${totalAttempts} attempts exhausted with network errors`);
...
Return only the JSON array.`;
  ...
  try {
    const { response: apiResp, retriesAttempted } = await callAnthropicWithRetry(env, {
      model: MODEL,
    ...
    if (!apiResp.ok) {
      ...
      console.error(`Anthropic API error after ${retriesAttempted} retries:`, apiResp.status, errText);
